...
const columns: DataTableColumns = [
  {
    key: 'name',
    title: () => {
      return <div class="ml-24px">菜单名称</div>
    },
  },
  {
    key: 'type',
    ...
    <BaseCard :title>
      ...
      >
        ...
        <template #operation="{ row }">
          ...
          >
            <CrudColumnBtn
              v-if="hasPermission('system:menu:add') && row.type !== 'permission'"
              type="primary"
              @click="onAdd({ parentId: row.id, type: row.type === 'catalog' ? 'menu' : row.type === 'menu' ? 'permission' : 'menu' })"
            >
              新增
            </CrudColumnBtn>
            <CrudColumnBtn
